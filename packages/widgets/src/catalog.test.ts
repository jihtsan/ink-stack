...
describe("widget catalog", () => {
  it("contains the image widget with consistent server renderers", () => {
    expect(widgetCatalog.map((definition) => definition.manifest.type)).toEqual(expect.arrayContaining(["text", "date", "todo", "codex-usage", "image"]));
    expect(widgetServerRegistry.map((definition) => definition.manifest.type).sort()).toEqual(widgetCatalog.map((definition) => definition.manifest.type).sort());
